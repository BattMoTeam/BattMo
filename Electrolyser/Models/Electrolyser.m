    ...
    methods
        ...
        function [model, state] = setupBcAndInitialState(model)
            ...
            for ielde = 1 : numel(eldes)
                ...
                H2Ogrho = H2Ovp*model.(elde).(ptl).species.H2O.molecularWeight./(con.R*T);
                ...
                switch elde
                  ...
                  case her
                    ...
                    H2p = pGas;
                    H2rho = H2p.*model.(elde).(ptl).species.H2.molecularWeight / (con.R * T);
                    state.(elde).(ptl).H2rhoeps = H2rho*gvf;

                  case oer

                    O2p = pGas;
                    O2rho = O2p.*model.(elde).(ptl).species.O2.molecularWeight / (con.R * T);
        ...
        function [state, report] = updateAfterConvergence(model, state0, state, dt, drivingForces)
            ...
            state = model.evalVarName(state, 'compGasMasses'); % remember that the argument is searched for in the string representation of the variable names
            state = model.evalVarName(state, 'liqrho');
