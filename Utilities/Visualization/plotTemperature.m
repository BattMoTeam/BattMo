...
%   Detailed explanation goes here
    ...
    if step ~= 0
        if model.grid.griddim == 1
            plotCellData(model.ThermalModel.grid, states{step}.ThermalModel.T - 273.15);
            xlabel(gca, 'Position  /  m')
            ylabel(gca, 'Temperature  /  ^{\circ}C')

        elseif model.grid.griddim == 2
            plotCellData(model.ThermalModel.grid, states{step}.ThermalModel.T - 273.15, 'edgealpha', 0.1);
            colormap(inferno);
            xlabel(gca, 'Position  /  m')
        ...
        for i = 1:length(states)
            if i == 1
                ...
                xmin = min(model.ThermalModel.grid.nodes.coords(:,1));
                xmax = max(model.ThermalModel.grid.nodes.coords(:,1));
                if model.grid.griddim == 2
                    ymin = min(model.ThermalModel.grid.nodes.coords(:,2));
                    ymax = max(model.ThermalModel.grid.nodes.coords(:,2));
                elseif model.grid.griddim == 3
                    ymin = min(model.ThermalModel.grid.nodes.coords(:,2));
                    ymax = max(model.ThermalModel.grid.nodes.coords(:,2));
                    zmin = min(model.ThermalModel.grid.nodes.coords(:,3));
                    zmax = max(model.ThermalModel.grid.nodes.coords(:,3));
                end
            else
        ...
        for i = 1:length(states)
            ...
            if model.grid.griddim == 1
                plotCellData(model.ThermalModel.grid, states{i}.ThermalModel.T - 273.15, 'LineWidth', style.lineWidth);
            ...
            elseif model.grid.griddim == 2
                plotCellData(model.ThermalModel.grid, states{i}.ThermalModel.T - 273.15, 'edgealpha', 0.1);
                colormap(inferno);
                xlabel(gca, 'Position  /  m')
                ylabel(gca, 'Position  /  m')
                title('Temperature  /  ^{\circ}C')
