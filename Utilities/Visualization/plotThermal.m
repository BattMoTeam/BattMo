...
%   Detailed explanation goes here
    ...
    for i = 1:length(states)
        ...
        if strcmpi(p.Results.docked, 'true')
            ...
            clim([min(states{1}.ThermalModel.jHeatSource), max(states{end}.ThermalModel.jHeatSource)]);
            colorbar();
            ...
            % Plot Temperature
            subplot(1,2,2), plotTemperature(model,states, 'step', i)
            colormap(gca, fs.colormap_temperature);
            clim([min(states{1}.ThermalModel.T), max(states{end}.ThermalModel.T)]);
            colorbar();
            cbar = get(gca, 'Colorbar');
            cbar.Color = fs.fontColor;
            ...
            % Plot Temperature
            figure(2), plotTemperature(model,states, 'step', i)
            colormap(gca, fs.colormap_temperature);
            clim([min(states{1}.ThermalModel.T), max(states{end}.ThermalModel.T)]);
            colorbar();
            cbar = get(gca, 'Colorbar');
