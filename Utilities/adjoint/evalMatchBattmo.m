...
if nargout > 1
    ...
    objh = @(tstep,model, state) obj(setupNew.model, states, setupNew.schedule, 'ComputePartials', true, 'tStep', tstep,'state', state);
        ...
        case 'PerturbationADNUM'
            ...
            try  % Try parallel loop
                parfor i=1:numel(p_org)
                    val(i) = evaluateMatchBattmo(perturb(p_org,i,eps_pert),...
