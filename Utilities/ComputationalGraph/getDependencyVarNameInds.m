function [depvarnameinds, propfuncinds, propvarnameinds, propdeplevels, staticinds, staticdeplevels] = getDependencyVarNameInds(varnameinds, A)
% The inputs are
...
%                     It is obtain by concatenating propvarnameinds and staticinds
...
%                     Same size as propfuncinds
% - staticinds      : Indices that depends on the indices given by varnameinds throught the adjency matrix A, but which corresponds to static variable
% - staticdeplevels : Level of dependency (increases by one for each evaluation of a PropFunction that is needed to obtain the PropFunction given by propfuncinds)
%                     Same size as staticinds
% 
    [propfuncinds   , ...
     propvarnameinds, ...
     propdeplevels  , ...
     staticinds     , ...
     staticdeplevels] =  getDependencyVarNameIndsRecursive(varnameinds, ...
                                                           []         , ... 
                                                           []         , ... 
                                                           []         , ... 
                                                           []         , ... 
...
% Helper function for recursive computation
    ...
    for ivar = 1 : numel(varnameinds)
        ...
        if all(c == 0)
            ...
            staticdeplevels = vertcat(staticdeplevels, level);
