    ...
    methods
        ...
        % See output structure defined at the end of the function
            ...
            for ielde = 1 : numel(eldes)
                ...
                f = model.(elde).(co).(am).(itf).computeOCPFunc.eval(c(1 : end - 1)/icmaxs{ielde});
                
                % function handler
                fs{ielde} = @(s) model.(elde).(co).(am).(itf).computeOCPFunc.eval(((1 - s).*c0s{ielde} + s.*cTs{ielde})/icmaxs{ielde});
            ...
            U = model.(pe).(co).(am).(itf).computeOCPFunc(cs.p0, 298, cmaxs.p) - model.(ne).(co).(am).(itf).computeOCPFunc.eval(cs.n0/cmaxs.n);
