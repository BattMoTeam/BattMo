classdef CO2captureControl < BaseModel
%%
% Class is taken from ControlModel
    
    methods

        function model = CO2captureControl(inputparams)
        ...
        % timeSteppingParams structure by default is given by the data described in :battmofile:`Utilities/JsonSchemas/TimeStepping.schema.json`
            ...
            params = ControlModel.parseTimeSteppingStruct(params);

            totalTime = getStructField(params, 'totalTime');

            assert(isAssigned(totalTime), 'totalTime must be given');
