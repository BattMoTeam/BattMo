...
\begin{tikzpicture}
  ...
  { 
    elde [as = \myobjnode{2.3}{\mcode{Electrode}\\ (\mcodeclass{Electrode})}] ->
      { am [as = \myobjnode{2.8}{\mcode{ActiveMaterial} \\ \mcodeclass{(ElectronicComponent)}}] -> 
        { itf [as = \myobjnode{1.7}{\mcode{Interface}\\ \mcodeclass{(Interface)}}], sd [as = \myobjnode{2.7}{\mcode{SolidDiffusion}\\ (\mcodeclass{SolidDiffusionModel})}]
          },
        cc [as = \myobjnode{2.8}{\mcode{CurrentCollector} \\ \mcodeclass{(ElectronicComponent)}}] ,
      }
    };
\end{tikzpicture}
