    ...
    methods
        ...
        function step = setupScheduleStep(model, timeSteppingParams)
            ...
            if isAssigned(params, {'totalTime'})
                ...
                % With a global totalTime, future generic control switches are not known when the schedule is
                % constructed. Take the min(dts) for safety.
                dt = min(dts);
                ...
                for ictrl = 1 : numel(controlsteps)
                    ...
                    if strcmp(termination.quantity, 'time')
                        ...
                        ctrlvals = repmat(dt, params.numberOfTimeSteps, 1);
                    end
