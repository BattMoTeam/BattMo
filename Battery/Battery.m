...
%
    properties
        ...
        % NE PE weights
        %length_factors = initVariablesADI([1.0, 1.0]);
        length_factors = [1.0, 1.0];
    end
    ...
    methods
        ...
        function model = Battery(paramobj)
            ...
            model.NegativeElectrode = Electrode(paramobj.NegativeElectrode, {model.length_factors(1), model.length_factors(2)});
            model.PositiveElectrode = Electrode(paramobj.PositiveElectrode, {model.length_factors(2), model.length_factors(1)});
            ...
            for k = 1:2
                ...
                cells = find(cell_idx);
                c_length_factor.(elde).cells = cells;
                c_length_factor.(elde).length_factor = model.length_factors(k);

                f = mcolon(paramobj.G.cells.facePos(cells), paramobj.G.cells.facePos(cells+1) - 1);
                cf_length_factor.(elde).cells = cells;
                cf_length_factor.(elde).cf = f;
                cf_length_factor.(elde).length_factor = model.length_factors(k);
